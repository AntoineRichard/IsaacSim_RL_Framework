...
if __name__ == "__main__":
    ...
    def train(config):
        ...
        test_envs = [make_env(config, writer, 'test', datadir, store=True)]
        ...
        while step < config.steps:
            ...
            simulate(functools.partial(agent, training=False), test_envs, episodes=1, step=step, target_vel=[[random.random()+0.3]])
            writer.flush()
            #print('Start collection.')
            #steps = config.eval_every // config.action_repeat
            #state = simulate(agent, train_envs, steps, state=state, step=step, target_vel=[[random.random()+0.3]])
            step += 500#count_steps(datadir, config)
