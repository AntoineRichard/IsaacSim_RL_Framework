...
class Turtlebot2Wrapper(BaseRobotWrapper):
    def __init__(self, stage):
        super().__init__(stage)
        self.asset_path = self.nucleus_server + "/RLEnvironments/Robots/turtlebot2_with_laser.usd"
        self.ar = None
        self.left_wheel_joints = ["wheel_left_joint"]
        self.right_wheel_joints = ["wheel_right_joint"]
        self.dist_chassis = 0.115
    ...
    def loadPlugins(self):
        ...
        self.base_link = "/turtlebot/base_link"
        # Add perfect pose sensor
        self.PPS = PerfectPoseSensor(self.stage, self.PhysXIFace, self.DCIFace, self.base_link)
        # Add lidar accessor
        self.lidar_path = "/turtlebot/camera_link/Lidar"
        self.activateLidar()

    def spawn(self, position):
        prims = []
        prims = utils.createObject('/turtlebot', self.stage, self.asset_path, False, position=position, group=prims, allow_physics=False)
        self.heron_prim = prims[0]

    def teleport(self, position, rotation, settle=False):
        if self.ar is None:
            self.ar = self.DCIFace.get_articulation("/turtlebot")
