...
class Turtlebot3Wrapper(BaseRobotWrapper):
    def __init__(self, stage):
        super().__init__(stage)
        self.asset_path = self.nucleus_server + "/RLEnvironments/Robots/turtlebot3_with_laser.usd"
        self.ar = None
        self.left_wheel_joints = ["wheel_left_joint"]
        self.right_wheel_joints = ["wheel_right_joint"]
        self.dist_chassis = 0.08
        self.wheel_radius = 0.033
        self.wheel_separation_multipier = 1.0
    ...
    def loadPlugins(self):
        ...
        self.base_link = "/turtlebot3_burger/base_link"
        # Add perfect pose sensor
        self.PPS = PerfectPoseSensor(self.stage, self.PhysXIFace, self.DCIFace, self.base_link)
        # Add lidar accessor
        self.lidar_path = "/turtlebot3_burger/base_scan/Lidar"
        self.activateLidar()

    def spawn(self, position):
        prims = []
        prims = utils.createObject('/turtlebot3_burger', self.stage, self.asset_path, False, position=position, group=prims, allow_physics=False)
        self.heron_prim = prims[0]

    def teleport(self, position, rotation, settle=False):
        if self.ar is None:
            self.ar = self.DCIFace.get_articulation("/turtlebot3_burger")
