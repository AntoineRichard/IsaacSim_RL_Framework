...
class HuskyWrapper(BaseRobotWrapper):
    def __init__(self, stage):
        super().__init__(stage)
        self.asset_path = self.nucleus_server + "/RLEnvironments/Robots/husky_with_laser.usd"
        self.ar = None
    ...
    def loadPlugins(self):
        ...
        self.base_link = "/husky/base_link"
        # Add perfect pose sensor
        self.PPS = PerfectPoseSensor(self.stage, self.PhysXIFace, self.DCIFace, self.base_link)
        # Add lidar accessor
        self.lidar_path = "/husky/base_laser/Lidar"
        self.activateLidar()

    def spawn(self, position):
        prims = []
        prims = utils.createObject('/husky', self.stage, self.asset_path, False, position=position, group=prims, allow_physics=False)
